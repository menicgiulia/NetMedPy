...
- Details about each function (what it is used for, what the input parameters are, the possible values of the input parameters, what the output is) from the pipeline are available in [Documentation](doc/build/html/NetMedPy.html) the `netmedpy/NetMedPy.py` script in the comments before each function. 
