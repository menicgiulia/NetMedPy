...
import distances.NetworkMetrics as new_distances
...
def exe_time(CONTEXT,s,reps,method,distMatrix = None):
    ...
    for i in range(reps):
        ...
        if method == 'optimized':
            c.tick()
            prox = new_distances.separation_z_score(ppi, A, B, distMatrix,'exact',1000,0)
            c.tock()
...
def create_execution_time_series(CONTEXT,size_ini,size_fin,size_step,method,reps,dmatrix=None):
    ...
    outfile = f"C:/Users/aalda/Dropbox (Personal)/Barabasi Lab/Postdoc/proximitySeparationOptimization/proximity/data/separation/OptimTime/{method}_{reps}.csv"
...
def plot_series(reps):
    ...
    dm = pd.read_csv(inFile + f"optimized_{reps}.csv")


    plt.figure()
    plt.plot(dm['size'],dm['time']+35,label=f"Optimized - {reps} reps",color = 'red', linewidth=2)
    plt.xlabel("A,B Size")
